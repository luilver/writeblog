...
class Leaf::PositionableTest < ActiveSupport::TestCase
  setup do
    @leaves = blogs(:handbook).leaves.positioned
  end
  ...
  test "new items are inserted at the end" do
    new_page = blogs(:handbook).press Page.new(body: "New Page"), title: "New Page"

    assert_equal new_page, blogs(:handbook).leaves.positioned.last
  end

  test "the first item in the collection has the expected score" do
    blogs(:handbook).leaves.destroy_all
    new_page = blogs(:handbook).press Page.new(body: "New Page"), title: "New Page"
