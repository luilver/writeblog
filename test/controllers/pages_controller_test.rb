...
class PagesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create" do
    post blog_pages_path(blogs(:handbook), format: :turbo_stream), params: { leaf: { title: "Another page" }, page: { body: "With interesting words." } }
    assert_response :success

    new_page = Page.last
    assert_equal "Another page", new_page.title
    assert_equal "With interesting words.", new_page.body.content
    assert_equal blogs(:handbook), new_page.leaf.blog
  end

  test "create with default params" do
    assert_changes -> { Page.count }, +1 do
      post blog_pages_path(blogs(:handbook), format: :turbo_stream)
    end
    assert_response :success
  ...
  test "create at a specific position" do
    assert_changes -> { Page.count }, +1 do
      post blog_pages_path(blogs(:handbook), format: :turbo_stream), params: { position: 2 }
    end
    assert_response :success

    assert_equal 2, blogs(:handbook).leaves.before(Page.last.leaf).count
  end
  ...
  private
    def sample_page_leaf(markdown)
      blogs(:handbook).press Page.new(body: markdown), title: "Sample"
    end
end
