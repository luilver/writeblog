module Blog::Accessable
  ...
  class_methods do
    def accessable_or_published(user: Current.user)
      if user.present?
        accessable_or_published_blogs
      else
        published
      end
    end

    def accessable_or_published_blogs(user: Current.user)
      user.blogs.or(published).distinct
    end
  end
  ...
  def update_access(editors:, readers:)
    ...
    accesses.upsert_all(all_accesses, unique_by: [ :blog_id, :user_id ])
