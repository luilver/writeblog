module BlogsHelper
  def blog_toc_tag(blog, &)
    tag.ol class: "toc", tabindex: 0,
      data: {
        ...
        arrangement_url_value: blog_leaves_moves_url(blog)
      }, &
  end

  def blog_part_create_button(blog, kind, **, &)
    url = url_for [ blog, kind.new ]
  ...
  def link_to_previous_leafable(leaf, hotkey: true, for_edit: false)
    if previous_leaf = leaf.previous
      ...
      link_to blog_slug_path(leaf.blog), data: hotkey_data_attributes("left", enabled: hotkey), class: "btn" do
        image_tag("arrow-left.svg", aria: { hidden: true }, size: 24) + tag.span("Table of contents: #{ leaf.blog.title }", class: "for-screen-reader")
      end
    end
  end
  ...
  def link_to_next_leafable(leaf, hotkey: true, for_edit: false)
    if next_leaf = leaf.next
      ...
      link_to blog_slug_path(leaf.blog), data: hotkey_data_attributes("right", enabled: hotkey), class: "btn txt-medium" do
        tag.span("Table of contents: #{leaf.blog.title }", class: "overflow-ellipsis") + image_tag("arrow-reverse.svg", aria: { hidden: true }, size: 24)
      end
    end
  end
